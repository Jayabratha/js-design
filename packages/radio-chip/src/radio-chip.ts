...
export class RadioChip extends LitElement {
    ...
    render() {
        return html`
            ...
            <div id='${this.id}' class='jsd-radio-chip-wrapper ${this.inline ? 'inline' : ''} ${this.theme} ${this.errorMsg ? 'error' : ''}'>
                <div class='chip-wrapper'>
                    ${this.list.map((item: any) => html`
                        <div class='jsd-radio-chip ${this.fullWidth ? 'full-width' : ''}'>
                            <input 
                            ...
                            <label for='${this.id}-${item.value !== undefined ? item.value : item}' 
                                   class='${this.disabled ? 'disabled' : ''}'>${unsafeHTML(item.label !== undefined ? item.label : item)}</label>
                        </div>
                    `)}
                </div>
